...
@Component({
    templateUrl: './edit-contract-kol-dialog.component.html',
    standalone: true,
    imports: [
        FormsModule,
...
export class EditContractKolDialogComponent extends AppComponentBase implements OnInit {
    @Output() onSave = new EventEmitter<any>();

    saving = false;
    entry = new ContractKolDto();
    id: string;
    ...
    ngOnInit(): void {
        this._kolService.getAll(undefined, undefined, undefined, undefined, 0, 1000).subscribe((r) => {
            this.kols = r.items ?? [];
            this._contractService.getAll(undefined, undefined, undefined, undefined, 0, 1000).subscribe((rc) => {
                this.contracts = rc.items ?? [];
                this._contractKolService.get(this.id).subscribe((ck) => {
                    this.entry = ck;
                    this.airTime = ck.airTime ? ck.airTime.toDate() : null;
    ...
    save(): void {
        ...
        this._contractKolService.update(this.entry).subscribe({
            next: () => {
